...
const addStaff = (req, res) => {
  const staffId = Math.floor(Math.random() * 10e5)
  let { surName, otherNames, email, phoneNo, address, password,gender,dateOfBirth, role, classTaken, subjectTaken,salary } = req.body;
...
const updateStaff = async (req, res) => {
  ...
  const updatestaff = await staffModel.findOneAndUpdate({staffId}, {
...
const getTeachers = async (req, res) => {
  try {
    ...
    res.status(500).send({ status: false, message: "An error occurred", error: error.message });
  }
};
...
module.exports = {getStaffsByRole, getTeachers, addStaff,getStaffs, login, updateStaff, deleteStaff, findStaffByEmail, findStaffById, findStaffBySurName, teachersDashboard }